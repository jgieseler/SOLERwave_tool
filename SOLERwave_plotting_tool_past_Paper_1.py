...
def oktant_plot(m_data_list,base_map,flare_coordinates,r_sun_ref,ttheta,aangles_along_arc,time,file_path_dict = [],save_path = [],filename_appendix = [],j_map_range=[0.75,1.25]):
    ...
    intensity_mean,intensity_median,intensity_var,pixel_per_segment,mask_3 = find_segments_from_list(theta_range,ttheta,angles_along_arc_range,aangles_along_arc,m_data_list)
...
def coord_for_sector(Flare_coordinates,theta_range,angles_along_arc_range):
    from SOLERwave_find_fit_great_arcs import find_coord_from_angles
...
def plot_timeseries_of_lineplot_and_map(segment_nr, theta_range, map_series, m_ref_height, intensity_mean, intensity_var, time, distance, linplot_ylim,
                                        ...
                                        filename_appendix = [], distance_marker = True,distance_marker_label=False, contrast_mode = False,
    ...
    for t in tqdm(time_index_range,desc = text,file=sys.stdout):
        ...
        if distance_marker:
            ...
            if distance_marker_label:
                ...
                coords = find_coord_from_angles(Flare_coordinates, theta_vec, aaa_100mM)
...
def plot_timeseries_of_lineplot_and_map_V2(segment_nr, theta_range, map_series, m_base, intensity_mean, intensity_var, time, distance, linplot_ylim,
                                        ...
                                        quicklook = False):
    """ Creates a Movie for 1 sector with the perturbation profile on the bottom and the Map on Top
...
def plot_fit_with_wave_features( time,
    ...
    for j in range(d_front_mat.shape[0]):
        ...
        if max_nr_peaks_vec[j] != 0:
            ...
            for nr_w in range(nr_of_waves_vec[j]):
                ...
                if (a[0] >= v_min_fit_Mm) and (a[0] <= v_max_fit_Mm):
                    ...
                    if not show_all_points:
                        ...
                        to_small_wavewidth = (min_wave_width<(delta_segment/2))
                        delta_wave_width[0,to_small_wavewidth] = wave_width[to_small_wavewidth]-delta_segment/2
            ...
            import matplotlib.dates as mdates
            from matplotlib.ticker import MultipleLocator, AutoMinorLocator
            locator = mdates.AutoDateLocator(minticks=5, maxticks=20)#Todo: maxticks were 12
            formatter = mdates.ConciseDateFormatter(locator)
